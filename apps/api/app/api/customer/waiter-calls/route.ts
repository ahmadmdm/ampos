...
export async function POST(req: NextRequest) {
  ...
  if (!body.branchId || !body.tableId || !body.reason) {
    return fail("branchId, tableId, reason are required", 400);
  }
  ...
  const cooldownSec = branch.waiterCallCooldownSec;
  const limiterKey = `waiter_call:${branchId}:${tableId}`;
  const allowed = await enforceRateLimit(limiterKey, cooldownSec);
  if (!allowed) {
    return fail("RATE_LIMITED_WAIT_60_SECONDS", 429);
  }

  const waiterCall = await prisma.$transaction(async (tx) => {
